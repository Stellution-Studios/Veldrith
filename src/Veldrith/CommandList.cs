...
namespace Veldrith
{
    ...
    public abstract class CommandList : IDeviceResource, IDisposable
    {
        ...
        ///     When drawing, the active <see cref="Pipeline" /> must be compatible with the bound <see cref="Veldrith.Framebuffer" />,
        ...
        ///     When drawing, the active <see cref="Veldrith.Framebuffer" /> must be compatible with the active <see cref="Pipeline" />.
        ...
        private void preDrawValidation()
        {
            ...
            if (Framebuffer == null) throw new VeldridException($"A {nameof(Veldrith.Framebuffer)} must be set in order to issue draw commands.");
